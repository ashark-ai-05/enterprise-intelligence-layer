...
async function probe(
  ...
  try {
    const response = await environment.fetchImpl(url, {
      ...init,
...
export function checkNodeVersion(environment: DoctorEnvironment): CheckResult {
  ...
  return {
    ...
    ...(ok
      ...
      : {
          implication:
            "The WASM and fetch behaviour this design relies on needs Node 20+.",
        }),
  };
}
    ...
    evidence: config.bypassAll
      ...
      : config.noProxy.length === 0
        ...
        : config.noProxy
            .map(
              (r) =>
                `${r.subdomainsOnly ? "." : ""}${r.host}${r.port === undefined ? "" : `:${r.port}`}`,
            )
            .join(", "),
  };
 ...
 * "the source is slow".
 */
export async function checkProxyDispatcher(
  environment: DoctorEnvironment,
): Promise<CheckResult> {
  const config = readProxyConfig(environment.env);
  ...
  try {
    ...
    return {
      ...
      ...(hasEnvAgent
        ...
        : {
            implication:
              "Without it, Node's fetch silently ignores HTTPS_PROXY and external hosts hang.",
          }),
    };
  } catch (error) {
    return {
      ...
      implication:
        "Node's fetch ignores HTTPS_PROXY without it. Every proxied source will time out.",
    };
  }
}
...
export async function checkHostReachable(
  ...
  const result = await probe(environment, target.url, {
    method: "GET",
    redirect: "manual",
  });
 ...
 * to establish.
 */
export async function checkMaasEmbeddings(
  environment: DoctorEnvironment,
): Promise<CheckResult> {
  ...
  if (base === undefined || base.trim() === "") {
    return {
      ...
      implication:
        "Until this is known, bulk embedding stays local (corpus egress argument, docs/15 §F2).",
    };
  }
  ...
  const result = await probe(environment, url, {
    ...
    body: JSON.stringify({
      input: "eil doctor capability probe",
      model: environment.env.EIL_MAAS_MODEL,
    }),
  });
...
export async function checkNativeBinaryDependency(): Promise<CheckResult> {
  try {
    ...
    return {
      ...
      status: "pass",
      evidence:
        "onnxruntime-node loaded — a native binary is present and usable",
    };
  } catch {
    return {
      id: "runtime.native",
