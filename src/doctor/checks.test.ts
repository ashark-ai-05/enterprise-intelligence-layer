...
import { readProxyConfig } from '../net/proxy.js';

const environmentWith = (
  overrides: Partial<DoctorEnvironment> & { env?: Record<string, string | undefined> },
): DoctorEnvironment => ({
  env: overrides.env ?? {},
  nodeVersion: overrides.nodeVersion ?? 'v22.0.0',
  fetchImpl: overrides.fetchImpl ?? (() => Promise.reject(new Error('no network in tests'))),
  now: overrides.now ?? (() => 0),
});
...
describe('checkNodeVersion', () => {
  it('passes on 20 and above', () => {
    expect(checkNodeVersion(environmentWith({ nodeVersion: 'v20.11.0' })).status).toBe('pass');
    expect(checkNodeVersion(environmentWith({ nodeVersion: 'v24.18.0' })).status).toBe('pass');
  });

  it('fails below 20 and says why it matters', () => {
    const result = checkNodeVersion(environmentWith({ nodeVersion: 'v18.19.0' }));
    expect(result.status).toBe('fail');
    expect(result.implication).toMatch(/Node 20/);
  });

  it('reports the observed version as evidence, not a verdict', () => {
    expect(checkNodeVersion(environmentWith({ nodeVersion: 'v22.1.0' })).evidence).toContain('v22.1.0');
  });
});
...
describe('checkProxyConfiguration', () => {
  ...
  it('renders the parsed NO_PROXY rules back, so a mis-parse is visible', () => {
    const checks = checkProxyConfiguration(
      environmentWith({ env: { NO_PROXY: '.corp.example,build.internal:8080' } }),
    );
  ...
  it('warns when NO_PROXY is unset that everything is proxied', () => {
    ...
    expect(checks.find((c) => c.id === 'proxy.no_proxy')!.evidence).toMatch(/every host goes through/);
  });
...
describe('targetsFromEnv', () => {
  ...
  it('picks up each configured source', () => {
    ...
    expect(targets.map((t) => t.id)).toEqual(['source.confluence', 'source.jira']);
  });
...
describe('checkHostReachable', () => {
  const target = { id: 'source.confluence', label: 'Confluence', url: 'https://wiki.corp.example/rest/api' };
  ...
  it('passes on any HTTP response — reaching the server is the fact being established', async () => {
    ...
    const result = await checkHostReachable(environment, target, readProxyConfig({}));
  ...
  it('reports a bypassed host as direct', async () => {
    const config = readProxyConfig({ HTTPS_PROXY: 'http://proxy:3129', NO_PROXY: '.corp.example' });
    const environment = environmentWith({
      fetchImpl: () => Promise.resolve(new Response(null, { status: 200 })),
      now: clock(10),
    });
