...
import { applyDiversityCap, DEFAULT_K, rrf, type Arm, type ArmHit } from './rrf.js';
...
describe('rrf', () => {
  ...
  it('sums contributions across arms', () => {
    const fused = rrf([arm('lexical', ['a', 'b']), arm('semantic', ['b', 'a'])]);
  ...
  it('ranks a document agreed on by two arms above one that leads a single arm', () => {
    ...
    const fused = rrf([arm('lexical', ['solo', 'agreed']), arm('semantic', ['other', 'agreed'])]);
  ...
  it('never compares raw scores across arms — only ranks', () => {
    ...
    expect(twoArms.map((f) => f.id)).toEqual(oneArmDoubleWeight.map((f) => f.id));
  ...
  it('is deterministic across permutations of equally-scored input', () => {
    ...
    expect(rrf([arm('a', ['z']), arm('b', ['y'])])).toEqual(rrf([arm('a', ['z']), arm('b', ['y'])]));
  });
  ...
  it('counts a duplicate id within one arm only once', () => {
    const withDuplicate = rrf([{ name: 'sloppy', hits: [hit('a'), hit('a'), hit('b')] }]);
  ...
  it('records per-arm contributions for explain output', () => {
    ...
    expect(top!.contributions.map((c) => c.arm)).toEqual(['lexical', 'semantic']);
  ...
  it('truncates each arm independently with perArmLimit', () => {
    const fused = rrf([arm('a', ['1', '2', '3']), arm('b', ['4', '5', '6'])], { perArmLimit: 1 });
...
describe('applyDiversityCap', () => {
  ...
  it('demotes rather than discards, so a dominated result set still fills', () => {
    const fused = fusedOf(hit('j1', 'jira'), hit('j2', 'jira'), hit('j3', 'jira'));
  ...
  it('leaves hits with no source or container uncapped', () => {
    ...
    expect(applyDiversityCap(fused, { maxPerSource: 1 }).map((f) => f.id)).toEqual(['a', 'b', 'c']);
  });
  ...
  it('applies the final limit after demotion', () => {
    const fused = fusedOf(hit('j1', 'jira'), hit('j2', 'jira'), hit('c1', 'confluence'));
