...
export interface FusedHit<T extends ArmHit = ArmHit> {
  ...
  readonly contributions: readonly { arm: string; rank: number; weight: number; score: number }[];
}
 ...
 * input ordering.
 */
export function rrf<T extends ArmHit>(arms: readonly Arm<T>[], options: RrfOptions = {}): FusedHit<T>[] {
  ...
  interface Accumulator {
    ...
    contributions: { arm: string; rank: number; weight: number; score: number }[];
  }
  const byId = new Map<string, Accumulator>();

  arms.forEach((arm, armIndex) => {
    const weight = arm.weight ?? 1;
    const hits = options.perArmLimit === undefined ? arm.hits : arm.hits.slice(0, options.perArmLimit);
    ...
    hits.forEach((hit, position) => {
      ...
      if (existing === undefined) {
        ...
        existing.contributions.push({ arm: arm.name, rank, weight, score: contribution });
      }
    });
  });
  ...
  return [...byId.entries()]
    ...
    .sort((a, b) => {
      ...
      if (a.acc.firstArmIndex !== b.acc.firstArmIndex) return a.acc.firstArmIndex - b.acc.firstArmIndex;
...
export function applyDiversityCap<T extends ArmHit>(
  ...
  for (const item of fused) {
    ...
    const sourceUsed = source === undefined ? 0 : (sourceCount.get(source) ?? 0);
    const containerUsed = container === undefined ? 0 : (containerCount.get(container) ?? 0);

    const overSource = maxPerSource !== undefined && source !== undefined && sourceUsed >= maxPerSource;
    const overContainer =
      maxPerContainer !== undefined && container !== undefined && containerUsed >= maxPerContainer;
    ...
    if (container !== undefined) containerCount.set(container, containerUsed + 1);
    kept.push(item);
  }
