...
import { isBypassed, parseNoProxy, readProxyConfig, shouldProxy } from './proxy.js';

const config = (env: Record<string, string | undefined>) => readProxyConfig(env);
...
describe('parseNoProxy', () => {
  ...
  it('splits on commas and whitespace', () => {
    ...
    expect(rules.map((r) => r.host)).toEqual(['a.example', 'b.example', 'c.example']);
  });
  ...
  it('lower-cases hosts so matching is case-insensitive', () => {
    expect(parseNoProxy('WIKI.Corp.Example').rules[0]!.host).toBe('wiki.corp.example');
  });
...
describe('readProxyConfig', () => {
  it('prefers lower-case env vars over upper-case', () => {
    const c = config({ https_proxy: 'http://lower:3128', HTTPS_PROXY: 'http://upper:3128' });
    expect(c.httpsProxy).toBe('http://lower:3128');
  });

  it('accepts upper-case when lower-case is absent', () => {
    expect(config({ HTTPS_PROXY: 'http://upper:3128' }).httpsProxy).toBe('http://upper:3128');
  });
...
describe('isBypassed', () => {
  ...
  it('matches subdomains of a dotted rule', () => {
    ...
    expect(isBypassed(new URL('https://deep.wiki.corp.example/x'), c)).toBe(true);
  });
  ...
  it('uses the protocol default port when the URL omits one', () => {
    ...
    expect(isBypassed(new URL('https://secure.internal/x'), withDefault)).toBe(true);
    expect(isBypassed(new URL('http://secure.internal/x'), withDefault)).toBe(false);
  });
...
describe('shouldProxy', () => {
  ...
  it('routes external https through the https proxy', () => {
    expect(shouldProxy('https://api.atlassian.net/rest', c)).toBe('http://proxy:3129');
  });
  ...
  it('goes direct for a NO_PROXY host — the case that silently hangs when this is wrong', () => {
    expect(shouldProxy('https://wiki.corp.example/rest/api', c)).toBeUndefined();
  });
  ...
  it('accepts a URL object as well as a string', () => {
    expect(shouldProxy(new URL('https://example.com/'), c)).toBe('http://proxy:3129');
  });
});
