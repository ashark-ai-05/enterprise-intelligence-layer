 ...
 */
export function parseNoProxy(raw: string | undefined): {
  rules: NoProxyRule[];
  bypassAll: boolean;
} {
...
export function isBypassed(target: URL, config: ProxyConfig): boolean {
  ...
  const port =
    target.port === "" ? defaultPortFor(target.protocol) : Number(target.port);
...
export async function installGlobalProxy(
  ...
  if (anyProxy === undefined) {
    return {
      installed: false,
      reason: "no HTTP_PROXY or HTTPS_PROXY set; using direct connections",
    };
  }
