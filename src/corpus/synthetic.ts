...
export function subjectMembers(
  ...
  if (index === undefined) {
    ...
    for (
      let i = 0;
      i < options.repositories * options.filesPerRepository;
      i += 1
    )
...
export function planLinks(options: SyntheticCorpusOptions): LinkPlan {
  ...
  for (let index = 0; index < options.jiraIssues; index += 1) {
    ...
    if (pageBucket === undefined || codeBucket === undefined)
      topicFallbacks += 1;
    ...
    const files =
      codeBucket ??
      Array.from({ length: codeCount }, (_, i) => codeIdAt(options, i));

    const pageId =
      pages[seededIndex(options.seed, `${issueKey}:page`, pages.length)];
    const codeId =
      files[seededIndex(options.seed, `${issueKey}:code`, files.length)];
...
export function generateSyntheticCorpus(
  ...
  for (
    let index = 0;
    index < options.confluencePages && denied < 20;
    index += 20
  ) {
